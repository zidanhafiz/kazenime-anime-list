	...
	return (
		<div className='flex gap-12 px-6 py-3 items-center bg-slate-100 dark:bg-gray-900 border rounded-md'>
			<div className='flex flex-col items-center justify-center border-r-2 pr-8'>
				...
				<p className='text-2xl font-semibold'>{anime ? anime?.score : manga && manga?.score}</p>
				<p className='my-1 text-xs text-center'>{anime ? anime?.scored_by?.toLocaleString() : manga && manga?.scored_by?.toLocaleString()} users</p>
			</div>
			<div>
				<div className='flex gap-8'>
					<p className='text-xl'>
						Ranked #<strong>{anime ? anime?.rank?.toLocaleString() : manga && manga?.rank?.toLocaleString()}</strong>
					</p>
					<p className='text-xl'>
						Popularity #<strong>{anime ? anime?.popularity?.toLocaleString() : manga && manga?.popularity?.toLocaleString()}</strong>
					</p>
					<p className='text-xl'>
						Favorites #<strong>{anime ? anime?.favorites?.toLocaleString() : manga && manga?.favorites?.toLocaleString()}</strong>
					</p>
				</div>
